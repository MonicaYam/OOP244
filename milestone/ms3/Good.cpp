...
;
	...
	fstream& Good::store(fstream& file, bool newLine) const {
		file << _typeOfGood << ','<< _sku << ',' << _unit << ',' << _goodName << ',' << _goodQtyHand << ',' 
			<< _goodQtyNeeded << ',' << _singlePrice << ',' << _taxable;
	...
	fstream& Good::load(fstream& file) {
		...
		if (file.is_open()) {
			file >>  tempGood._typeOfGood >> comma >> tempGood._sku >> comma 
				>> tempGood._unit >> comma >> tempGood._goodName >> comma>> tempGood._goodQtyHand >> comma
				>> tempGood._goodQtyNeeded >> comma>> tempGood._singlePrice >> comma >> tempGood._taxable;
		...
		if (isClear() && !isEmpty()) {
			if (linear) {
				...
				if (strlen(_goodName) > 74) {
					os << "Name(no spaces): " << setw(74) << _goodName << endl;
				}
				else {
				...
				os << "Quantity needed: " << _goodQtyNeeded << endl;
